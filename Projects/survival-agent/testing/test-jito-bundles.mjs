...
class JitoBundleTester {
   ...
   */
  async buildBundle(swapTransaction, tipLamports) {
   ...
   */
  async sendBundlePaperMode(bundle) {
...
async function main() {
  ...
  // Use paper trading RPC and wallet (same env vars as paper-trade-bot)
  const rpcUrl = process.env.HELIUS_RPC_URL || 'https://api.mainnet-beta.solana.com';
  const privateKey = process.env.SOLANA_PRIVATE_KEY;

  if (!privateKey) {
    console.error('❌ SOLANA_PRIVATE_KEY environment variable not set');
    console.error('   Set with: export SOLANA_PRIVATE_KEY=<your-private-key>');
    process.exit(1);
  }
